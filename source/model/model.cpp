...
#include "../eiq/neutron/driver/include/NeutronDriver.h"
...
constexpr int kTensorArenaSize = (340) * 1024;
static const float anchor1[3][6] = {
#include "./anchors.txt"
};
// An area of memory to use for input, output, and intermediate arrays.
// (Can be adjusted based on the model needs.)

static uint8_t s_tensorArena[kTensorArenaSize]  __attribute__((aligned(64)));

status_t MODEL_Init(void)
{
    NeutronError neutron_err = neutronInit();
    if (neutron_err != ENONE)
    {
        PRINTF("neutronInit() failed: %d\r\n", neutron_err);
        return kStatus_Fail;
    }
